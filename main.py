...
def fetch_pull_requests(token, repo, state, date_field, per_page=100):
    ...
    while True:
        ...
        for pr in prs:
            if state == "closed" and date_field == "merged_at":
                if pr.get("merged_at"):
                    prs_list.append((repo, pr.get("number"), pr.get("title"), pr.get("merged_at")))
            else:
                prs_list.append((repo, pr.get("number"), pr.get("title"), pr.get(date_field)))
        page += 1
...
def main():
    ...
    for repo in REPOS:
        merged_prs = fetch_pull_requests(token, repo, state="closed", date_field="merged_at")
        open_prs = fetch_pull_requests(token, repo, state="open", date_field="created_at")
